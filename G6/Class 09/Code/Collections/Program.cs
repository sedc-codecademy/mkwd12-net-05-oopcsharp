...
ourQueue.Enqueue(5);//add 5 to the end of the queue
ourQueue.Enqueue(6);//add 6 to the end of the queue

int firstElement = ourQueue.Dequeue(); //remove and return the first element (in our case that is the 100)
Console.WriteLine(firstElement);
int readFirstMemeber = ourQueue.Peek(); //only get the value of the first member, but do not remove it from the queue
Console.WriteLine(readFirstMemeber);
...
string lastElement = ourStack.Pop();//remove and return the last member (that will be bye in our case)
Console.WriteLine(lastElement);

string currentLastElement = ourStack.Peek(); //read but do not remove the last member
